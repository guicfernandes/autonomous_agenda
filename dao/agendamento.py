"""Module to handle Appointment agendamento data access object (DAO) operations"""

from datetime import datetime, timedelta

from connection.db_connection import close_connection, get_cursor, start_connection
from dao.user import get_user
from entities.agendamento import Appointment
from entities.user import User
from utils.exceptions import (
    AppointmentNotFoundException,
    NoAppointmentsForSpecifiedPeriod,
...
def get_user_appointment(user: User, date: datetime = None) -> Appointment:
    ...
    Date could be provided to get appointment for a given date:
        - if provided it should contain appointment's date and hour.
...
def list_appointments(
    ...
    We could fetch:
        - only past events;
        - only future events
        - only tomorrow's reminder events
        - only today events
        - all events from the database using bool variables.
    ...
    if rows:
        ...
        for row in rows:
            appointment = Appointment(
                id=row[0], client=get_user(user_id=row[1]), date=row[2]
            )
            appointments.append(appointment)
    else:
        if in_past:
            message = f"No past appointments when comparing to date: {current_date.strftime('%Y-%m-%d')}"
        elif in_future:
            message = f"No future appointments when comparing to date: {current_date.strftime('%Y-%m-%d')}"
        elif is_reminder:
            message = f"No appointments reminder for date: {reminder_date.strftime('%Y-%m-%d')}"
        elif is_today:
            message = f"No appointments for today: {current_date.strftime('%Y-%m-%d')}"
        else:
            message = "No appointments found."
        raise NoAppointmentsForSpecifiedPeriod(message=message)
    return appointments
...
def delete_appointment(
    client_name: str, client_email: str, appointment_date: str
) -> None:
    ...
    try:
        ...
        try:
            ...
            conn.commit()
            print(
                f"Agendamento de {appointment_date} do usuário {client_name} de e-mail {client_email} deletado com sucesso do banco de dados."
            )
        except AppointmentNotFoundException as e:
...
def update_user_appointment(
    client_name: str, client_email: str, current_date: str, new_date: str
) -> None:
    ...
    try:
        ...
        try:
            ...
            conn.commit()
            print(
                f"Agendamento de {current_date} do usuário {client_name} de e-mail {client_email} atualizado com sucesso para {new_date}."
            )
        except AppointmentNotFoundException as e:
